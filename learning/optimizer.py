...
def _has_select_dropdown(soup: BeautifulSoup) -> str | None:
    for sel in ["select#chapterList", "select.chapter-select",
                "select[name='chapter']", "select.selectpicker", "select#chapter"]:
...
def _score_fetch_strategy(config: PipelineConfig, is_js_heavy: bool) -> float:
    ...
    if first_type == "curl":
        return 0.7 if is_js_heavy else 1.0
    if first_type == "hybrid":
        return 0.85
...
class PipelineGenerator:
    def generate(
        ...
        for i, cs in enumerate(content_sels[:3]):
            ...
            cand = self._build_selector_variant(domain, cs, ns, ts, is_js_heavy, label=f"selector_v{i+1}")
        ...
        print(f"  [Optimizer] 📋 Generated {len(candidates)} candidate pipelines", flush=True)
        return candidates

    def _build_focused(self, domain, is_js_heavy, has_json_ld, has_rel_next,
                       dropdown_sel, content_sels, next_sels, title_sels) -> PipelineConfig:
        fetch_steps = (
    ...
    def _build_selector_variant(self, domain, content_sel, next_sel, title_sel, is_js_heavy, label="") -> PipelineConfig:
        fetch_steps = (
            [StepConfig("playwright"), StepConfig("hybrid")]
            if is_js_heavy
            else [StepConfig("curl"), StepConfig("playwright")]
        )
        extract_steps = [StepConfig("selector", {"selector": content_sel}), StepConfig("density_heuristic"), StepConfig("fallback_list")]
        title_steps: list[StepConfig] = []
        if title_sel:
        ...
        return PipelineConfig(
            ...
            title_chain   = ChainConfig("title",    title_steps),
            nav_chain     = ChainConfig("navigate", nav_steps),
            validate_chain= ChainConfig("validate", [StepConfig("length", {"min_chars": 100}), StepConfig("prose_richness", {"min_word_count": 20})]),
            notes = label,
        )

    def _build_json_ld_focused(self, domain, is_js_heavy, next_sels) -> PipelineConfig:
        fetch_steps = [StepConfig("playwright")] if is_js_heavy else [StepConfig("curl"), StepConfig("playwright")]
        nav_steps   = [StepConfig("rel_next")]
        if next_sels:
            nav_steps.append(StepConfig("selector", {"selector": next_sels[0]}))
        nav_steps += [StepConfig("anchor_text"), StepConfig("slug_increment"), StepConfig("fanfic"), StepConfig("ai_nav")]
        return PipelineConfig(
            domain        = domain,
            fetch_chain   = ChainConfig("fetch",    fetch_steps),
            extract_chain = ChainConfig("extract",  [StepConfig("json_ld"), StepConfig("density_heuristic"), StepConfig("fallback_list")]),
            title_chain   = ChainConfig("title",    [StepConfig("h1_tag"), StepConfig("title_tag"), StepConfig("og_title"), StepConfig("url_slug")]),
            nav_chain     = ChainConfig("navigate", nav_steps),
            validate_chain= ChainConfig("validate", [StepConfig("length", {"min_chars": 100}), StepConfig("prose_richness", {"min_word_count": 20})]),
    ...
    def _build_dropdown_variant(self, domain, is_js_heavy, dropdown_sel, content_sels) -> PipelineConfig:
        ...
        return PipelineConfig(
            ...
            nav_chain     = ChainConfig("navigate", [
                ...
                StepConfig("rel_next"), StepConfig("anchor_text"),
                StepConfig("slug_increment"), StepConfig("fanfic"), StepConfig("ai_nav"),
            ]),
            validate_chain= ChainConfig("validate", [StepConfig("length", {"min_chars": 100}), StepConfig("prose_richness", {"min_word_count": 20})]),
            notes = "dropdown_nav",
        )
...
class PipelineEvaluator:
    ...
    async def evaluate(
        ...
        results: list[CandidateResult] = await asyncio.gather(*tasks, return_exceptions=False)

        for i, (result, config) in enumerate(zip(results, candidates)):
            print(
                ...
                f"resource={result.resource_score:.2f}",
                flush=True,
            )
    ...
    async def _eval_one(
        ...
        total      = 0.4 * quality + 0.3 * speed + 0.2 * blended_resource + 0.1 * confidence
        ...
        return CandidateResult(
            ...
            resource_score = round(blended_resource, 3),
...
def _merge_ai_selectors(config: PipelineConfig, profile: dict) -> None:
    ...
    if ai_content:
        ...
        if "selector" not in existing_types:
            config.extract_chain.steps.insert(0, StepConfig("selector", {"selector": ai_content}))
        else:
    ...
    if ai_next:
        for step in config.nav_chain.steps:
            ...
            config.nav_chain.steps.insert(insert_at, StepConfig("selector", {"selector": ai_next}))
    ...
    if ai_title:
        for step in config.title_chain.steps:
            ...
            config.title_chain.steps.insert(0, StepConfig("selector", {"selector": ai_title}))
...
def _config_duplicate(new: PipelineConfig, existing: list[PipelineConfig]) -> bool:
    ...
    return any(e.to_dict() == new_dict for e in existing)
